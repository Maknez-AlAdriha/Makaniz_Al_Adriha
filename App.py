...
def generate_printable_html(name, s_type, region, province, loc, hist, daily, annual, books, creative, links, beliefs_text, source_val="رواية شفوية ميدانية مأثورة"):
    ...
    return html_content
# ==========================================
# 🏛️ الجزء 5: تأسيس وهيكلة جداول قاعدة البيانات الكبرى مع الترقية التلقائية الفورية للأعمدة
# ==========================================
def init_ultimate_db():
    ...
    # 🟢 حل سحري جذري لخطأ لافتة العلبة الحمراء: حقن العمود في قاعدة البيانات السحابية القديمة تلقائياً إن لم يكن موجوداً لمنع أي انقطاع
    try:
...
with btn_col3:
    if st.button("📬 دَفْتَر التَّوَاصُل وِالمُلَاحَظَات", key="contact_page_btn"):
        ...
        st.rerun()
# 🟢 فك تشنج السيرفر: سحب عناصر التوبل بمؤشر الفهرس [0] لإبطال خطأ TypeError وتأمين استقرار العدادات الأربعة
...
# تفريغ مربعات اللوحة الإحصائية التراكمية المحدثة لزوار الأطروحة الوطنية
...
# ==========================================
# 🌿 الجزء 7: لوحة التحليلات الأنثروبولوجية المطهرة (دمج التكرار وقهر بتر الكلمات)
# ==========================================
if menu == "🔍 محرك البحث الشامل والتحليلات":
    ...
    with analysis_col1:
        ...
        if functions_fetch:
            # تحويل البيانات إلى جدول مرن
            df_chart = pd.DataFrame(functions_fetch, columns=["نوع الوظيفة الأنثروبولوجية", "عدد المزارات"])
            
            # 🟢 تطهير مجهري وصارم: سحق المسافات الخفية لدمج أصناف "القداسة والشرف" المكررة تلقائياً صلب الموضوع
            ...
            # 🟢 قهر بتر الحروف: تحويل المخطط إلى نظام أفقي (Horizontal) لتظهر الكلمات الطويلة كاملة مائة بالمائة أمام لجان المناقشة
            st.bar_chart(df_chart, x="عدد المزارات", y="نوع الوظيفة الأنثروبولوجية", use_container_width=True)
        else: 
            st.caption("سيرتفع الرسم البياني فور ضخ جداول الكرامات الشفوية.")
    ...
    with analysis_col2:
        ...
        if quick_word:
            if "مريد" in quick_word: 
                st.info("🔗 **مفاهيم هرمية ذات صلة سلوكية وميدانية مقترحة:** الشيخ ⟵ الخلوة ⟵ السلوك ⟵ الحضرة ⟵ الذاكرة الشعبية")
            elif "هيلولة" in quick_word or "ربيين" in quick_word: 
                st.info("🔗 **مفاهيم هرمية ذات صلة عبرية مقترحة:** مزارات اليهود ⟵ زيارة جماعية ⟵ تبادل القداسة ⟵ الصنف التجاري")
            else: 
                st.caption("المصطلح نشط، وسيتم استدعاء شجرته الهرمية الكبرى فور تغذية الخزانة اللغوية.")
    ...
    # استكمال فلاتر ومحددات البحث العلمي الشامل والمساعد المفهومي السريع والمطهر واجهياً مائة بالمائة
    st.write("---")
    with st.container():
        ...
        if quick_search_input:
            ...
            if term_fetch: 
                st.info(f"📙 **التصنيف الموسوعي:** {term_fetch[0]} \n\n 📝 **الشرح المفاهيمي الصافي:** {term_fetch[1]}")
            else:
                ...
                if shrine_fetch: 
                    st.info(f"📍 **الموقع الترابي الميداني:** {shrine_fetch[0]} \n\n 📜 **الترجمة والترجمة النَّسبية الكاملة:** {shrine_fetch[1]}")
                else: 
                    st.caption("هذا المصطلح أو المعلم غير مدرج حالياً في مكنزك الوطني.")
...
# ==========================================
    ...
    with st.form("add_shrine_ultimate_form"):
        ...
        with col1:
            ...
            # 🟢 التحسين 5: حقن خانة التوثيق المرجعي (scientific_source) بداخل فورم الادخال اليدوي لضمان رصانة التحقيق
...
# ==========================================
# 🎓 الجزء 9: لوحة الشرف الأكاديمية وبطاقة التعريف ومستودع تواصل الباحثين المرفوع للقمة
# ==========================================
if menu == "🎓 حول المكنز الأكاديمي":
    ...
    <div style='background: linear-gradient(135deg, #FFFDF6, #F9F5E8); border: 3px double #D4AF37; padding: 35px; border-radius: 15px; text-align: center; margin-bottom: 25px;'>
        ...
        <div style='background-color: #1E3A8A; color: white; padding: 10px 25px; display: inline-block; border-radius: 8px; font-weight: bold; font-size: 20px; margin: 10px auto;'>
            👨‍🎓 الباحث الدكتور: رشيد الجانبي
        </div>
        <br>
        <div style='background-color: #D4AF37; color: #1E3A8A; padding: 8px 25px; display: inline-block; border-radius: 8px; font-weight: bold; font-size: 19px; margin: 5px auto; border: 1px solid #B4912D;'>
            👩‍🏫 الأستاذة المشرفة: الدكتورة فاطنة الغزي
        </div>
        <p style='font-size: 18px; color: #4B5563; font-weight: bold; margin-top: 15px; margin-bottom: 5px;'>🏛️ المضمون المؤسسي للأطروحة:</p>
