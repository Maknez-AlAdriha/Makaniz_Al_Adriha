...
# 🎨 2. حقن كود الـ CSS الصافي والمعزول كلياً لتفادي تداخل الأقواس وسحق الـ SyntaxError
st.markdown("""
    <style>
        @import url('https://googleapis.com');
        
        /* نسف وتصغير كافة الهوامش والفراغات الجانبية في الحاويات الأم للمنصة */
...
# 🏢 3. رسم صف الأزرار الخمسة أولاً في قمة الواجهة
...
with menu_col_1:
    ...
    if st.button("الرئيسية", key="btn_home_clean_v1"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    
with menu_col_2:
    st.markdown("<div class='shamel-nav-btn'>", unsafe_allow_html=True)
    if st.button("أقسام المكنز", key="btn_sections_clean_v1"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    
with menu_col_3:
    st.markdown("<div class='shamel-nav-btn'>", unsafe_allow_html=True)
    if st.button("حول المكنز", key="btn_about_clean_v1"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    
with menu_col_4:
    st.markdown("<div class='shamel-nav-btn'>", unsafe_allow_html=True)
    if st.button("اتصل بنا", key="btn_contact_clean_v1"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    
with menu_col_5:
    st.markdown("<div class='shamel-nav-btn'>", unsafe_allow_html=True)
    if st.button("🔍 شعار البحث في المكنز", key="btn_search_clean_v1"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)

# 🏢 4. محرك استدعاء وعرض الصورة الملكية مباشرة أسفل الأزرار بنظام الـ Base64 الصافي
target_banner = None
