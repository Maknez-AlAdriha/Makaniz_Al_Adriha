...
# 🎨 2. حقن كود الـ CSS الجراحي لجعل الأزرار تندمج في أعلى الواجهة بشكل فني
st.markdown("""
    <style>
        @import url('https://googleapis.com');
        
        /* تصفير الهوامش الخارجية البيضاء لالتصاق الواجهة بأطراف الشاشة تماماً */
        ...
        /* إلغاء الفراغات العمودية التلقائية بين المكونات */
        div[data-testid="stVerticalBlock"] { gap: 0rem !important; }
        
        /* تنسيق الأزرار الخمسة لتظهر كروابط علوية رشيقة منسجمة مع ألوان الصورة */
        .shamel-nav-btn button {
            background: rgba(0, 0, 0, 0.6) !important; /* خلفية داكنة ناعمة تبرز الحروف بوضوح */
            color: #FFFFFF !important; /* خط أبيض ناصع وبارز */
            font-family: 'Tajawal', sans-serif !important;
            font-weight: 700 !important;
            font-size: 16px !important;
            border: 1px solid rgba(255, 255, 255, 0.3) !important;
            border-radius: 4px !important; /* حواف رقيقة كلاسيكية */
        ...
        /* تأثير الوميض الزمردي الشريف للمملكة عند تمرير مؤشر الفأرة */
...
# 🟢 الخطوة الحاسمة: رسم صف الأزرار أولاً في أعلى المتصفح قبل عرض الصورة
...
with menu_col_1:
    ...
    if st.button("الرئيسية", key="btn_home_top"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    
with menu_col_2:
    st.markdown("<div class='shamel-nav-btn'>", unsafe_allow_html=True)
    if st.button("أقسام المكنز", key="btn_sections_top"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    
with menu_col_3:
    st.markdown("<div class='shamel-nav-btn'>", unsafe_allow_html=True)
    if st.button("حول المكنز", key="btn_about_top"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    
with menu_col_4:
    st.markdown("<div class='shamel-nav-btn'>", unsafe_allow_html=True)
    if st.button("اتصل بنا", key="btn_contact_top"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)
    
with menu_col_5:
    st.markdown("<div class='shamel-nav-btn'>", unsafe_allow_html=True)
    if st.button("🔍 شعار البحث في المكنز", key="btn_search_top"):
        st.rerun()
    st.markdown("</div>", unsafe_allow_html=True)

# 🏢 3. محرك استدعاء وعرض الصورة الملكية مباشرة أسفل صف الأزرار العلوي
target_banner = None
